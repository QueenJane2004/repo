...
def user_dashboard():
    ...
    # Simple recommendations: books user hasn't borrowed yet
...
def admin_dashboard():
    ...
    return render_template(
        ...
        trending_books = trending_books or [],
...
def add_book():
    ...
    db.query_db(
        "INSERT INTO books (title, author, publisher, image_url, barcode, quantity, status) VALUES (%s,%s,%s,%s,%s,%s,'Available')",
        (title, author, publisher, image_url, barcode, quantity)
    )
...
def view_users():
    all_users = db.query_db("SELECT * FROM users ORDER BY id DESC")
    return render_template('view_users.html', all_users=all_users or [])
...
@login_required
def get_messages():
    # Lightweight message history — return empty list if table not set up
    return jsonify([])
