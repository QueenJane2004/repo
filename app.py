...
def login():
    ...
    if request.method == 'POST':
        ...
        # Plain text query: no hashing used
        user = db.query_db(
...
def register():
    ...
    if request.method == 'POST':
        ...
        # Storing password directly as plain text
        db.query_db(
...
def user_dashboard():
    ...
    recommendations = []
    try:
        # Note: RANDOM() for SQLite, use RAND() for MySQL
...
def transactions_log():
    ...
    if is_admin:
        # Note: CONCAT(u.firstname, ' ', u.lastname) for MySQL
        # Note: u.firstname || ' ' || u.lastname for SQLite
